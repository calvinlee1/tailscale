...
// TODO: MAKE SURE LOGIN MODE WORKS EVEN WHEN ACCESSED OVER TAILSCALE
// TODO: MAKE SURE HTTPS CONNECTION CHECK WORKS
// TODO: ALSO CHECK THAT EACH PAGE GETS PERMS

function PopoverContentHeader({ auth }: { auth: AuthResponse }) {
  return (
    <div className="text-black text-sm font-medium leading-tight mb-1">
      {auth.authorized ? "Viewing" : "Managing"}
  ...
  return (
    <>
      ...
      {!hasAnyPermissions ? (
        ...
        <p className="text-gray-500 text-xs">
          You don’t have permission to make changes to this device, but you can
          view most of its details.
        </p>
      ) : (
        !auth.authorized && (
